...
V3: Get ALL transactions and filter by swap events, not by source.
"""
...
def fetch_all_transactions(wallet, before_sig=None):
    ...
    params = {
        "api-key": HELIUS_KEY,
        "limit": 100,
        # No source or type filter - get everything
    }
    if before_sig:
...
def extract_swap_from_transaction(tx):
    ...
    # Method 1: Check for swap events
    ...
    if swap:
        ...
        if native_input and token_outputs:
            ...
            for i, (token_in, token_out) in enumerate(zip(token_inputs[:1], token_outputs[:1])):
                ...
                # Try to fetch metadata if needed
...
def transform_helius_to_walletdoctor(wallet, output_file):
    ...
    print(f"\nProcessed {total_tx} total transactions")
    print(f"Found {len(all_trades)} swap trades")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python helius_to_walletdoctor_v3.py <wallet_address> [output_file.csv]")
        sys.exit(1)
