...
Full version: Fetches complete transaction history with better progress tracking.
"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data and len(data) > 0:
                ...
                symbol = None
                if metadata.get("onChainMetadata"):
                    symbol = metadata["onChainMetadata"].get("metadata", {}).get("data", {}).get("symbol")
                if not symbol and metadata.get("legacyMetadata"):
                    symbol = metadata["legacyMetadata"].get("symbol")
                if symbol:
                    TOKEN_MINTS[mint_address] = symbol
                    return symbol
    except:
        pass
    return None
...
def fetch_all_transactions(wallet, before_sig=None):
...
def extract_swap_from_transaction(tx):
    """Extract swap information from any transaction type."""
    trades = []
    ...
    # Method 1: Check transaction type
    if tx.get("type") == "SWAP":
        # Try to get swap details from events
        ...
        # Parse native and token inputs/outputs
        ...
        if native_input and token_outputs:
            ...
            for token_out in token_outputs:
                ...
                if symbol == mint[:6] + "...":
                    fetched_symbol = get_token_metadata(mint)
                    if fetched_symbol:
            ...
            for i, (token_in, token_out) in enumerate(zip(token_inputs[:1], token_outputs[:1])):
                ...
                symbol_in = get_token_symbol(mint_in)
                symbol_out = get_token_symbol(mint_out)
                
                if symbol_in == mint_in[:6] + "...":
                    fetched = get_token_metadata(mint_in)
                    if fetched: symbol_in = fetched
                if symbol_out == mint_out[:6] + "...":
                    fetched = get_token_metadata(mint_out)
                    if fetched: symbol_out = fetched
                ...
                price_in = get_usd_price(mint_in, timestamp)
                price_out = get_usd_price(mint_out, timestamp)
                ...
                if price_in:
                    ...
                    if symbol_in == 'USDC' or symbol_in == 'USDT':
                        value_usd = amount_in
                    elif symbol_out == 'USDC' or symbol_out == 'USDT':
                        value_usd = amount_out
                    else:
                        return trades  # Skip if we can't determine value
...
def transform_helius_to_walletdoctor(wallet, output_file, max_pages=None):
    """Main transformation function."""
    print(f"Fetching FULL transaction history for wallet: {wallet}")
    ...
    print("(This may take several minutes for active wallets)")
    ...
    while True:
        ...
        if page_count % 10 == 0:
            print(f"  Progress: {total_tx} transactions processed, {total_swaps} swaps found...")
        
        transactions = fetch_all_transactions(wallet, last_sig)
        if not transactions:
            print(f"\nNo more transactions after page {page_count}")
            break
        ...
        for tx in transactions:
            ...
            # Extract swaps from this transaction
            trades = extract_swap_from_transaction(tx)
            if trades:
                total_swaps += 1
                source = tx.get('source', 'Unknown')
                source_counts[source] += len(trades)
        ...
        if len(transactions) < 100:
            print(f"\nReached end of history at page {page_count}")
            break
    ...
    print(f"\n{'='*60}")
    print(f"Processed {total_tx} total transactions across {page_count} pages")
    print(f"Found {total_swaps} swap transactions")
    print(f"Extracted {len(all_trades)} individual trades")
    
    print("\nTransaction type breakdown:")
    for tx_type, count in sorted(type_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
    ...
    print(f"\nUnique tokens traded: {len(trades_by_token)}")
    ...
    for trade in trades_with_pnl:
        trade['amount'] = round(trade['amount'], 8)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python helius_to_walletdoctor_full.py <wallet_address> [output_file.csv] [max_pages]")
        print("  max_pages: Optional limit for testing (e.g., 50)")
        sys.exit(1)
    
    wallet = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else f"{wallet[:8]}_trades_full.csv"
    max_pages = int(sys.argv[3]) if len(sys.argv) > 3 else None
    
    try:
        transform_helius_to_walletdoctor(wallet, output_file, max_pages)
