...
import asyncio
import json
from blockchain_fetcher_v3 import BlockchainFetcherV3, fetch_wallet_trades_v3
...
async def test_v3_fetcher():
    ...
    # 5. Pricing working
    unpriced_pct = ((summary['total_trades'] - summary['priced_trades']) / summary['total_trades'] * 100) if summary['total_trades'] > 0 else 0
    ...
    # Parse rate
    parse_rate = (metrics['signatures_parsed'] / metrics['signatures_fetched'] * 100) if metrics['signatures_fetched'] > 0 else 0
    ...
    if result['trades']:
        ...
        for i, trade in enumerate(result['trades'][:5]):
            print(f"  {i+1}. {trade['timestamp']} - {trade['action']} {trade['amount']:.4f} {trade['token']} @ ${trade['price'] or 0:.4f}")
