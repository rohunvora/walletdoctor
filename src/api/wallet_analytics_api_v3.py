...
def calculate_simple_analytics(trades: list) -> dict:
    ...
    if not trades:
        return {"total_trades": 0, "total_volume": 0, "message": "No trades found"}
...
async def fetch_and_analyze(wallet_address: str):
    ...
    async with BlockchainFetcherV3Fast(
        progress_callback=lambda msg: logger.info(msg), skip_pricing=False, max_pages=10  # Limit for API response time
    ) as fetcher:
